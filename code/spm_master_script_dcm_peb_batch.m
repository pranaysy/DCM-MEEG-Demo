...
% Load estimated GCM (containing .Ep)
model = load(char(fullfile(out_dir, sprintf('GCM_%s.mat', name_tag))));
GCM = model.GCM;

% Clear out 2nd column with reduced models
GCM(:, 2) = [];

% Fill in 2nd column with models without Forward connections in B-matrix
for k=1:16
    GCM{k, 2} = DCM;
end

% Save GCM
gcm_forwardOff_file = fullfile(out_dir, 'GCM_ForwardOFF_Specification.mat');
save(gcm_forwardOff_file, 'GCM')
...
% Fill in 2nd column with models without Backward connections in B-matrix
for k=1:16
    GCM{k, 2} = DCM;
end

% Save GCM
gcm_backwardOff_file = fullfile(out_dir, 'GCM_BackwardOFF_Specification.mat');
save(gcm_backwardOff_file, 'GCM')
...
% Fill in 2nd column with models without Lateral connections in B-matrix
for k=1:16
    GCM{k, 2} = DCM;
end

% Save GCM
gcm_lateralOff_file = fullfile(out_dir, 'GCM_LateralOFF_Specification.mat');
save(gcm_lateralOff_file, 'GCM')
...
% Fill in 2nd column with models without Self connections in B-matrix
for k=1:16
    GCM{k, 2} = DCM;
end

% Save GCM
gcm_selfOff_file = fullfile(out_dir, 'GCM_SelfOFF_Specification.mat');
save(gcm_selfOff_file, 'GCM')
