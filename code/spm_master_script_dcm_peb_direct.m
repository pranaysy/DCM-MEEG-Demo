...
% Save full model
DCM_Full = DCM;
dcm_full_file = fullfile(base_dir, 'fits', 'script', strcat(DCM.name, '_Specification.mat'));
...
% Save reduced model
dcm_self_file = fullfile(base_dir, 'fits', 'script', strcat(DCM.name, '_Specification.mat'));
...
for f=1:length(input_files)
    ...
    DCM_this = DCM_Full; % Full model
    DCM_this.xY.Dfile = input_files{f};
    GCM{f, 1} = DCM_this;
    
    DCM_this = DCM_Self; % Reduced model
    DCM_this.xY.Dfile = input_files{f};
    GCM{f, 2} = DCM_this; 
...
%---------------------------------------------------------------------------------------
% STEP 1: Manage outputs
%---------------------------------------------------------------------------------------

% Specify output directory
out_dir = fullfile(base_dir, 'fits', 'script');

% Switch to working dir
cd(out_dir)

% Save specification
save('GCM_Specification', 'GCM')
